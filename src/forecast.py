...
def bootstrap_metrics(y_true, y_pred, n_bootstraps=5000, ci_level=95):
    ...
    for _ in range(n_bootstraps):
        ...
        if np.any(mask):
            ...
            mape_s = (
                np.mean(np.abs((yt_sample[mask] - yp_sample[mask]) / yt_sample[mask]))
                * 100
            )
            mae_bs.append(mae_s)
...
def identify_peaks(df, response_variable, window=19):
    ...
    """
    rolling_max = (
        df[response_variable].rolling(window=window, min_periods=1, center=False).max()
    )
...
def add_peak_features(df, response_variable):
    ...
    for i in range(len(df)):
        if peaks.iloc[i]:
            last_peak_idx = i
        df.at[i, "days_since_last_peak"] = (
            i - last_peak_idx if last_peak_idx >= 0 else np.nan
        )
...
def create_features(df, response_variable, product_cols):
    ...
    if not isinstance(df["Day of Week"].dtype, pd.CategoricalDtype):
        df["Day of Week"] = pd.Categorical(
            df["Day of Week"], categories=DAY_NAMES, ordered=True
        )

    df["rolling_mean_same_day_4weeks"] = df.groupby("Day of Week")[
        response_variable
    ].transform(lambda x: x.shift(6).rolling(window=24, min_periods=1).mean())
    ...
    df["rolling_mean_prev_day_4weeks"] = df.groupby("Prev Day Name")[
        response_variable
    ].transform(lambda x: x.shift(6).rolling(window=24, min_periods=1).mean())
    df["ema_prev_day_4weeks"] = df.groupby("Prev Day Name")[
        response_variable
    ].transform(lambda x: x.shift(6).ewm(span=24, min_periods=1, adjust=False).mean())
...
def train_and_forecast(
    ...
    product_cols = [
        c for c in base_numeric_cols if c not in drop_cols and c != response_variable
    ]

    df = df.drop(
        columns=[c for c in ALL_FC_COLUMNS if c in df.columns], errors="ignore"
    )
    ...
    for week_num in range(1, forecast_weeks + 1):
        ...
        model_all, params_all = train_model(
            X_train_full, y_train_full, n_trials=n_trials, device=device
        )
        ...
        mae_w, rmse_w, mape_w, r2_w = compute_metrics(
            original_actuals_week, predictions
        )
        print(
            f"\n===== Forecasting Week {week_num} ({start_date.date()} to {end_date.date()}) ====="
        )
        print(
            f"MAE: {mae_w:.2f}, RMSE: {rmse_w:.2f}, MAPE: {mape_w:.2f}%, R2: {r2_w:.4f}"
        )
        mae_ci_w, rmse_ci_w, mape_ci_w = bootstrap_metrics(
            original_actuals_week, predictions, n_bootstraps, ci_level
        )
        ...
        if fc_available and existing_forecast_week is not None:
            ...
            fc_mae_w, fc_rmse_w, fc_mape_w, fc_r2_w = compute_metrics(
                original_actuals_week, existing_forecast_week
            )
            print(
                f"Existing {fc_column} - MAE: {fc_mae_w:.2f}, RMSE: {fc_rmse_w:.2f}, MAPE: {fc_mape_w:.2f}%, R2: {fc_r2_w:.4f}"
            )

        week_pred_csv = os.path.join(
            output_dir,
            f'predictions_week_{week_num}_{response_variable.replace(" ","_")}.csv',
        )
        ...
        importance_df = pd.DataFrame(
            {"Feature": model_feature_names, "Importance": fi}
        ).sort_values(by="Importance", ascending=False)

        fi_csv_path = os.path.join(
            output_dir,
            f'feature_importance_week_{week_num}_{response_variable.replace(" ","_")}.csv',
        )
        ...
        df = df_original.copy()
        df = pd.merge(
            df, updated_response, on="Date", how="left", suffixes=("_orig", "")
        )
    ...
    if len(overall_actuals) > 0:
        ...
        final_mae, final_rmse, final_mape, final_r2 = compute_metrics(
            overall_actuals_array, overall_predictions_array
        )
        print(
            f"\n===== Overall Performance Across All {forecast_weeks} Forecasted Weeks ====="
        )
        print(
            f"Overall - MAE: {final_mae:.2f}, RMSE: {final_rmse:.2f}, MAPE: {final_mape:.2f}%, R2: {final_r2:.4f}"
        )
        mae_ci, rmse_ci, mape_ci = bootstrap_metrics(
            overall_actuals_array, overall_predictions_array, 5000, 95
        )
        ...
        print(
            f"95% Confidence Interval for MAPE: [{mape_ci[0]:.2f}%, {mape_ci[1]:.2f}%]"
        )
        ...
        if fc_available and overall_fc_forecasts is not None:
            ...
            fc_mae, fc_rmse, fc_mape, fc_r2 = compute_metrics(
                overall_actuals_array, overall_fc_array
            )
            print(
                f"\n===== Comparison with Existing Fc Forecast ({fc_column}) Across All Forecasted Weeks ====="
            )
            print(
                f"Existing {fc_column} - MAE: {fc_mae:.2f}, RMSE: {fc_rmse:.2f}, MAPE: {fc_mape:.2f}%, R2: {fc_r2:.4f}"
            )

        final_predictions_df = final_predictions_df.reset_index(drop=True)
        final_pred_csv_path = os.path.join(
            output_dir, "final_predictions_all_weeks.csv"
        )
    ...
    return {
        ...
        "predictions": (
            final_predictions_df if len(overall_actuals) > 0 else pd.DataFrame()
        ),
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train and recursively forecast, ensuring only one set of lagged response variables and removing response_variable_orig."
    )
    parser.add_argument(
        "--dataset", type=str, required=True, help="Path to dataset CSV file."
    )
    parser.add_argument(
        "--response", type=str, required=True, help="Response variable to predict."
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        "--output_dir", type=str, default=".", help="Directory for outputs."
    )
    parser.add_argument(
        "--forecast_weeks", type=int, default=3, help="Number of weeks to forecast."
    )
    parser.add_argument(
        "--device",
        type=str,
        default="cpu",
        choices=["cpu", "cuda"],
        help="XGBoost device. Defaults to cpu.",
    )
    ...
    print(
        f"MAE: {model_mae:.2f}, RMSE: {model_rmse:.2f}, MAPE: {model_mape:.2f}%, R2: {model_r2:.4f}"
    )
